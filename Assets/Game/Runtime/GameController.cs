...
namespace FrogCart.Runtime
{
    ...
    public sealed class GameController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Пауза между укусами: язык обязан долететь и вернуться, плюс мгновение
        /// на передышку.
        ///
        /// Была константой 0.28 — и это ломало картинку. У языка ход туда и обратно
        /// занимает tongueOut + tongueBack, а в ассете пользователя tongueOut стоял
        /// 0.6 с давнего эксперимента. Вагонетка стреляла снова через 0.28, то есть
        /// вдвое раньше возврата: язык перезапускался на полпути и никогда не
        /// доигрывал. Со стороны это выглядит дрожанием, а не выстрелом.
        ///
        /// Теперь пауза выводится из самого языка, и рассинхрона быть не может,
        /// какие бы числа ни стояли в конфиге.
        /// </summary>
        ...
        void RetireUselessCarts()
        {
            for (int i = 0; i < _slots.Length; i++)
            {
                ...
                // Места этой вагонетки в очередь не возвращаются, в отличие от
                // связки: возить ей нечего, и вернувшаяся заняла бы место на
                // контуре навсегда — тот самый тупик, от которого этот метод и
                // защищает. Уходит она с пустой доской по своему цвету, так что
                // терять нечего: непотраченные места были лишними изначально.
        ...
        ///
        /// **Места партнёра при этом не пропадают** — см. <see cref="ReturnSeatsToQueue"/>.
        /// Уводить с контура вагонетку вместе с недоеденным цветом значит уносить
        /// ёмкость с уровня, а ёмкости считаются место в место.
        /// </summary>
        void RetireLinkedPartners(int slot)
        {
            ...
            for (int i = 0; i < _slots.Length; i++)
            {
                ...
                ReturnSeatsToQueue(i);
                StartCoroutine(ReplaceCart(i));
            }
        }

        /// <summary>
        /// Вернуть в очередь места вагонетки, которую уводят с контура непустой.
        ///
        /// Без этого связка ломала уровень насмерть. Ёмкости уровня считаются по
        /// числу ударов место в место (`FoodHuntJsonConverter.Save`): запаса нет
        /// ни у одного цвета, и любое потерянное место делает свой цвет
        /// недоедаемым. Партнёр по связке уходит не тогда, когда опустел сам,
        /// а когда опустела вторая вагонетка, и уносил с собой всё несъеденное —
        /// на 106-м уровне это 117 блоков из 735 при формально сходящемся балансе.
        /// Проверка проигрыша молчала: она считает ёмкость, а ёмкость исчезала
        /// уже после её вопроса.
        ///
        /// Сама связка остаётся как была: место на контуре освобождается сразу,
        /// и пара по-прежнему стоит игроку темпа и лишнего запуска. Пропадает
        /// только потеря ёмкости.
        ///
        /// Возвращается в голову очереди, а не в хвост: очередь бывает за сотню
        /// вагонеток, и с хвоста возвращённая пропала бы из виду.
        ///
        /// Номер связки снимается — она уже сработала. Иначе вернувшаяся вагонетка
        /// увела бы с контура следующую пару, которой на этом уровне уже нет.
        /// </summary>
        void ReturnSeatsToQueue(int slot)
        {
            int left = _slots[slot].Count;
            if (left <= 0) return;

            // Счётчик слота обнуляется здесь же. CheckLose складывает ёмкость живых
            // слотов с очередью, а вагонетка живёт ещё столько, сколько едет к
            // выходу: не обнули — те же места посчитались бы дважды, и проверка
            // проигрыша на эти доли секунды была бы слепа.
            _slots[slot].Count = 0;

            _queue.Insert(_queueIndex, new LevelData.CartDef
            {
                colorId = _slots[slot].ColorId,
                capacity = left,
            });
