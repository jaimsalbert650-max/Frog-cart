...
namespace FrogCart.Data
{
    ...
    public sealed class GameConfig : ScriptableObject
    {
        [Header("Движение")]
        /// <summary>
        /// Скорость по рельсам. Верхняя граница поднята с 60 до 90: 60 упиралось
        /// в потолок ползунка, а просьбы «ещё быстрее» на этом не кончились.
        /// </summary>
        [Range(0f, 90f)] public float railSpeed = 64f;
        ...
        /// <summary>
        /// Сколько длится выезд вагонетки из очереди на контур.
        ///
        /// Отдельно от cartEnter намеренно: cartEnter — это появление вагонетки на
        /// месте, а здесь она проделывает путь через пол-экрана, и на 0.30 бросок
        /// читался рывком. Пока она летит, на рельсе её ещё нет: настоящая
        /// появляется ровно в момент приземления, иначе на экране две одинаковых.
        /// </summary>
        [Range(0.2f, 1.2f)] public float cartLaunch = 0.5f;
