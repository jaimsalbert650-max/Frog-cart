...
public static class LevelPackBuilder
{
    ...
    ///
    /// **Раскладка блоками по цвету.** Сперва головной ряд — по одной вагонетке
    /// каждого цвета, — потом все остальные первого цвета, второго и так далее;
    /// внутри цвета в порядке дробления.
    ///
    /// Раньше здесь была круговая раскладка, ради выбора: в любом окне очереди
    /// видно все цвета сразу и есть из чего выбрать размер. Довод верен для
    /// двадцати вагонеток и разваливается на ста двадцати — а столько их теперь
    /// и есть. Искать нужный цвет в перемешанной сотне это не выбор, а перебор:
    /// игрок знает, какой цвет ему нужен, и ищет его глазами по всей площадке.
    /// Блоками он берётся подряд.
    ///
    /// Цвета идут по номерам, а номера раздаются в порядке обхода картинки —
    /// первым идёт цвет верхнего левого блока. Это не идеальный порядок
    /// «снаружи внутрь», но и не случайный: верхний край картинки открыт
    /// с первого кадра.
    ///
    /// Открыт наружу ради <see cref="FoodHuntJsonConverter"/>: тот выдавал по одной
    /// вагонетке на цвет, и уровни с механиками стояли в паке особняком — очередь
    /// из семи штук на 903 места вместо сотни. Дробление одно на оба конвертера,
    /// иначе фактура очереди зависела бы от того, каким путём пришёл уровень.
    /// В counts может лежать что угодно, что меряется местами: у пака это блоки,
    /// у JSON-конвертера удары с учётом прочности.
    /// </summary>
    public static List<LevelData.CartDef> SplitIntoCarts(int[] counts, int colorCount)
    {
        // Набор номиналов задаёт всю фактуру очереди: есть на что разменять
        // остаток и есть что поставить «на подольше».
        //
        // Номиналы вдвое мельче прежних (были 30/8/18/5/24/12/15/6): вагонеток стало
        // примерно вдвое больше, и очередь чаще требует решения. Средний размер упал
        // с 14.75 до 7.6, то есть уровень на 310 блоков разбирается сорока с лишним
        // вагонетками вместо двадцати двух.
        int[] cycle = { 14, 4, 9, 3, 12, 6, 8, 5 };

        // Огрызок тоже уменьшен: при мелких номиналах прежние четыре доливались
        // почти всегда и съедали половину разнообразия.
        const int MinTail = 3;
        ...
        // Головной ряд: по одной вагонетке каждого цвета, дальше блоками.
        //
        // Ряд нужен не для красоты, а чтобы уровень не запирался. Очередь
        // показывает 128 вагонеток (Queue3DView.MaxMinis), а их бывает 222;
        // сложи цвета подряд — и на семи уровнях из двадцати четырёх последние
        // цвета начинаются за окном и не тапаются вовсе. Дальше как повезёт:
        // если открытые блоки как раз того цвета, игрок занимает контур
        // вагонетками, которым нечего есть, они не пустеют никогда, очередь
        // стоит — тупик, при котором проверка проигрыша молчит.
        //
        // Девять вагонеток в голове снимают это подчистую: любой цвет уровня
        // доступен с первого кадра, каким бы длинным ни был хвост.
        for (int color = 1; color <= colorCount; color++)
        {
            if (perColor[color].Count == 0) continue;

            queue.Add(new LevelData.CartDef
            {
                colorId = color,
                capacity = perColor[color][0],
            });
        }

        for (int color = 1; color <= colorCount; color++)
        for (int k = 1; k < perColor[color].Count; k++)
        {
            queue.Add(new LevelData.CartDef
            {
                colorId = color,
